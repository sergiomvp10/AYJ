...
function AdminPanel({ spots, onRefresh, adminPassword }: { spots: Spot[]; onRefresh: () => void; adminPassword: string }) {
  ...
  const filtered = searchQuery.trim()
    ? takenSpots.filter(s =>
        ...
        s.buyer_city.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleEdit = (spot: Spot) => {
    ...
    setEditCity(spot.buyer_city)
    setEditPayment(spot.payment_method)
    setAdminError('')
    setAdminSuccess('')
  }
  ...
  const handleSaveEdit = async () => {
    ...
    if (!editName.trim() || !editPhone.trim()) {
      setAdminError('Nombre y teléfono son obligatorios')
      return
    }
    setSubmitting(true)
    setAdminError('')
    try {
      const res = await fetch(`${API_URL}/api/admin/spots/${editSpot.number}`, {
        ...
        body: JSON.stringify({ buyer_name: editName, buyer_phone: editPhone, buyer_city: editCity, payment_method: editPayment })
      })
      if (!res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <input
        ...
        placeholder="Buscar por nombre, teléfono, ciudad o número..."
            ...
            <div key={spot.number} className="bg-gray-800 border border-gray-700 rounded-xl p-4 flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">{spot.buyer_phone} · {spot.buyer_city}</p>
                  <p className="text-xs text-gray-500">{spot.payment_method}</p>
                </div>
              </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={editPayment} onChange={e => setEditPayment(e.target.value)}
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-amber-500" />
              </div>
...
function App() {
  ...
  const handleReserve = async () => {
    ...
    if (!buyerName.trim() || !buyerPhone.trim() || !buyerCity.trim() || !paymentMethod.trim()) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/spots`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ number: selectedSpot, buyer_name: buyerName, buyer_phone: buyerPhone, buyer_city: buyerCity, payment_method: paymentMethod })
      })
      if (!res.ok) {
        const data = await res.json()
  ...
  const handleSelectSpot = (num: number) => {
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <header className="bg-gray-900 border-b border-gray-800">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <img src="/gordotech-logo.jpg" alt="GORDOTECH" className="h-14 rounded" />
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => { setShowAdmin(!showAdmin); if (showAdmin) { setAdminAuthenticated(false); setLoginPassword('') }; setView('home') }}
        ...
        {showAdmin ? (
          ...
          <>
            ...
            <div className="text-center">
              <button
                onClick={() => { setView('select'); setError(''); setSuccess('') }}
                disabled={loading || availableCount === 0}
                className="bg-amber-500 text-gray-900 px-10 py-4 rounded-2xl text-xl font-bold hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? 'Cargando...' : availableCount === 0 ? 'Todos los puestos vendidos' : 'Comprar mi puesto'}
              </button>
              {availableCount > 0 && !loading && (
                <p className="mt-3 text-gray-500 text-sm">Selecciona tu número de la suerte</p>
              )}
            </div>
          </>
        ) : view === 'select' ? (
          <div>
            <button onClick={handleBackToHome} className="flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
              Volver
            </button>
            <h2 className="text-2xl font-bold text-amber-400 mb-2">Escoge tu número</h2>
            <p className="text-gray-400 mb-6">Los números en gris están disponibles. Los rojos ya están vendidos.</p>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-2">
                {spots.map((spot) => (
                  <button
                    key={spot.number}
                    onClick={() => !spot.taken && handleSelectSpot(spot.number)}
                    disabled={spot.taken}
                    className={`
                      ...
                      ${spot.taken
                        ...
                        : 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-green-900/40 hover:border-green-500 hover:text-green-400 cursor-pointer'
                      }
                    `}
                  >
                    {spot.number}
          ...
          <div>
            <button onClick={() => { setView('select'); setSelectedSpot(null); setError('') }} className="flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
              Cambiar número
            </button>
            ...
            <div className="max-w-md mx-auto">
              <div className="bg-gray-900 border border-amber-500/30 rounded-2xl p-6">
                <div className="text-center mb-6">
                  <div className="inline-flex items-center justify-center bg-amber-500 text-gray-900 text-4xl font-bold w-20 h-20 rounded-2xl mx-auto mb-3">
                    {selectedSpot}
                  </div>
                  <h3 className="text-xl font-bold text-amber-400">Puesto #{selectedSpot}</h3>
                  <p className="text-gray-400 text-sm">$80.000 COP</p>
                </div>

                {error && (
                  <div className="bg-red-900/50 border border-red-700 text-red-300 px-4 py-3 rounded-xl mb-4 text-sm">
                    {error}
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    >
                      {submitting ? 'Reservando...' : 'Confirmar compra'}
                    </button>
                  </div>
                </div>
              </div>
      ...
      <footer className="bg-gray-900 border-t border-gray-800 mt-12 py-6 text-center text-gray-500 text-sm">
        ...
        <p className="mt-1">Rifa MacBook Air 13" M4 &middot; 100 puestos &middot; $80.000 COP c/u</p>
      </footer>
    </div>
  )
}
